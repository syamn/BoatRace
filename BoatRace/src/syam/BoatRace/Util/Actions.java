...
public class Actions {
	...
	public Actions(BoatRace plugin){
		this.plugin = plugin;
	}
	...
	public static void message(CommandSender sender, Player player, String message){
		if (message != null){
			message = message
					...
					.replaceAll("%version", BoatRace.getInstance().getDescription().getVersion());
			if (player != null){
...
//		EconomyResponse r = BoatRace.economy.depositPlayer(name, amount);
...
//		if (amount < 0) return false; // 負数は許容しない
//		EconomyResponse r = BoatRace.economy.withdrawPlayer(name, amount);
//		if(r.transactionSuccess()) {
//			return true;
//		} else {
//			return false;
...
//		return (BoatRace.economy.has(name, amount));
//	}
//
//
//	/****************************************/
